...
if (user_size % HEADER_SIZE == 0) {
  return HEADER_SIZE * 2 + user_size;
}
return HEADER_SIZE * 2 + user_size + (HEADER_SIZE - (user_size % HEADER_SIZE));
}
...
void myheap_free(struct myheap *h, void *payload) {
  ...
  set_block_header(get_block_start(payload), get_block_size(get_block_start(payload)), 0);
  coalesce(h, get_block_start(payload));
  if (is_first_block(h, get_block_start(payload)) == 0) {
      coalesce(h, get_previous_block(get_block_start(payload)));
  }
}
  /*
...
void *myheap_malloc(struct myheap *h, unsigned int user_size) {
  ...
  void *addy = h->start;
  while (is_within_heap_range(h, addy) == 1) {
    if (get_block_size(addy) >= get_size_to_allocate(user_size)) {
      if (block_is_in_use(addy) == 0) {
       return split_and_mark_used(h, addy, get_size_to_allocate(user_size));
      }
    } else {
      addy = addy + get_block_size(addy);
    }
  }
  return NULL;
}
